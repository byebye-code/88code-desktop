    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
        <div class="flex gap-3">
          <Info class="text-blue-600 flex-shrink-0" :size="20" />
...
const handleAdvancedConfig = async (config) => {
  ...
  try {
    // 使用高级配置命令，传递完整的配置内容
    const result = await invoke('configure_claude_advanced', {
...
const handleDeleteConfig = async () => {
  if (!confirm('确定要删除 Claude Code 配置文件吗？\n\n这将清空所有配置，包括 API 密钥、Base URL 等。此操作不可恢复。')) {
    return;
  }

  try {
    const result = await invoke('delete_claude_config');
    emit('success', result);
  } catch (error) {
    emit('error', error);
  }
